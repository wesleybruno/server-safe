...
- Portainer e Traefik rodam via `docker run` direto — se `docker` não estiver no PATH, chamam `ensure_docker` (helper novo em `common.sh`, extraído de `docker-install.sh` — o mesmo botão "instalar docker" e essas duas libs chamam a mesma função, sem duplicar a lógica de instalar/habilitar/validar). UI de administração (Portainer 9443, dashboard do Traefik 8080) amarrada em `127.0.0.1` — mesma postura do próprio painel server-safe, acesso só via túnel SSH; portas de dado do Traefik (80/443) ficam públicas porque é o trabalho dele.
...
- Como qualquer um desses 4 rodar com sucesso garante que o Docker está presente (já estava, ou acabou de ser instalado), o handler Go deles (`h.markOKWithDocker`, `internal/api/handlers.go`) marca `extras_docker` no progress junto com o próprio step — senão o check do card do Docker ficaria incoerente com o estado real da máquina.
