...
6. Step novo em `web/index.html` (sempre acessível, sem `disabled` — se depender de outra etapa, adiciona um `<p class="warn">` explicando) + wiring em `web/app.js` (`unlock(sel)` no callback de sucesso só navega até o próximo step, não trava nada).
