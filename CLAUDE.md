...
"Extras" é uma categoria, não um módulo só: cada lib (Docker, Portainer, Traefik, Coolify, EasyPanel, cPanel, Dozzle, Restic, Uptime Kuma, cAdvisor, Prometheus, Grafana, Loki, Zabbix) ganha seu próprio par script/módulo/rota/botão dentro do mesmo step `step-extras`, não um framework genérico de "instalar lib X". Roda antes da Auditoria de propósito — instalar software (repos novos, serviços novos escutando) é algo que a auditoria final deve enxergar.

UI da etapa 7 é agrupada por categoria (`<h3 class="extra-category">` + um `.extra-grid` por grupo, não um grid único): Containers (Docker/Portainer/Traefik), Painéis de hospedagem (Coolify/EasyPanel/cPanel), Logs (Dozzle/Loki), Monitoramento (cAdvisor/Prometheus/Grafana/Uptime Kuma/Zabbix), Backup (Restic). Categoria nova = repetir o padrão `<h3 class="extra-category">Nome</h3>` seguido de um `<div class="extra-grid">` só com os cards daquele grupo — não é um componente JS, é estrutura HTML pura.
...
- Como qualquer uma dessas 11 rodar com sucesso garante que o Docker está presente (já estava, ou acabou de ser instalado), o handler Go delas (`h.markOKWithDocker`, `internal/api/handlers.go`) marca `extras_docker` no progress junto com o próprio step — senão o check do card do Docker ficaria incoerente com o estado real da máquina.
