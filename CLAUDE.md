...
| `POST /api/extras/uptimekuma` | `InstallUptimeKuma` | `uptimekuma.go` |
| `POST /api/extras/cadvisor` | `InstallCadvisor` | `cadvisor.go` |
| `POST /api/extras/prometheus` | `InstallPrometheus` | `prometheus.go` |
| `POST /api/extras/grafana` | `InstallGrafana` | `grafana.go` |
| `POST /api/extras/loki` | `InstallLoki` | `loki.go` |
| `POST /api/extras/zabbix` | `InstallZabbix` | `zabbix.go` |
| `POST /api/extras/signoz` | `InstallSignoz` | `signoz.go` |
...
"Extras" é uma categoria, não um módulo só: cada lib (Docker, Portainer, Traefik, Coolify, EasyPanel, cPanel, Dozzle, Restic, Uptime Kuma, cAdvisor, Prometheus, Grafana, Loki, Zabbix, SigNoz) ganha seu próprio par script/módulo/rota/botão dentro do mesmo step `step-extras`, não um framework genérico de "instalar lib X". Roda antes da Auditoria de propósito — instalar software (repos novos, serviços novos escutando) é algo que a auditoria final deve enxergar.

UI da etapa 7 é agrupada por categoria (`<h3 class="extra-category">` + um `.extra-grid` por grupo, não um grid único): Containers (Docker/Portainer/Traefik), Painéis de hospedagem (Coolify/EasyPanel/cPanel), Logs (Dozzle/Loki), Monitoramento (cAdvisor/Prometheus/Grafana/Uptime Kuma/Zabbix/SigNoz), Backup (Restic). Categoria nova = repetir o padrão `<h3 class="extra-category">Nome</h3>` seguido de um `<div class="extra-grid">` só com os cards daquele grupo — não é um componente JS, é estrutura HTML pura.

UI da etapa 7: `.extra-card` em grid (`#step-extras .extra-grid`), um card por lib — ícone de marca real (SVG inline, baixado uma vez de `cdn.jsdelivr.net/npm/simple-icons` e colado no `index.html`, **não** carregado do CDN em runtime) ou monograma (`.extra-icon-mono`) quando a marca não tem ícone lá (EasyPanel, Dozzle, Restic — não estão no simple-icons). Sem badge de texto "não instalado" — só um `svg.check-icon` que aparece (`hidden` removido) quando a lib já está instalada, via `app.js`/`setExtraStatuses`/`EXTRA_KEYS` (lista separada do mapa `EXTRAS` de URLs, porque o Restic tem wiring próprio — não é um botão genérico — mas ainda precisa aparecer no check de status). `setExtraInstalled(key, installed)` é o único ponto que mexe em check+botão — some com `#btn-install-<key>` quando `installed` é true, pra não sobrar botão de instalar de algo que já está instalado. **Exceção: Restic não segue essa regra** — o botão dele é "aplicar configuração" (repo/paths/agenda podem mudar depois), não um instalador de uma vez só, então continua visível mesmo com o check marcado.
...
- Portainer, Traefik, Dozzle, Uptime Kuma, cAdvisor, Prometheus, Grafana, Loki e Zabbix rodam via `docker run` direto — se `docker` não estiver no PATH, chamam `ensure_docker` (helper em `common.sh`, extraído de `docker-install.sh` — o mesmo botão "instalar docker" e essas nove libs chamam a mesma função, sem duplicar a lógica de instalar/habilitar/validar).
...
- SigNoz é o único que não sobe via `docker run` nem instalador vendor `curl | sh` direto — usa `foundryctl`, a CLI oficial do projeto (instalada via `curl https://signoz.io/foundry.sh | bash` se ainda não tiver, em `/usr/local/bin`), que gera e sobe um docker-compose próprio (`/etc/server-safe/signoz/pours/deployment/compose.yaml`) a partir de um `casting.yaml` que o script escreve. `signoz-install.sh` também requer Docker (`ensure_docker`), então entra na mesma lista de dependência.
- Como qualquer uma dessas 12 rodar com sucesso garante que o Docker está presente (já estava, ou acabou de ser instalado), o handler Go delas (`h.markOKWithDocker`, `internal/api/handlers.go`) marca `extras_docker` no progress junto com o próprio step — senão o check do card do Docker ficaria incoerente com o estado real da máquina.
...
**Mapa de portas loopback dos extras** (`127.0.0.1` só, sempre — ver "Não fazer"): Portainer `9443`, Traefik dashboard `8090` (não `8080` — já é o painel server-safe; já rolou colisão real por isso, cuidado ao adicionar um extra novo, confere contra este mapa antes de escolher porta), Dozzle `8081`, Uptime Kuma `8082`, cAdvisor `8083`, Prometheus `9090` (padrão da ferramenta), Grafana `3000` (padrão da ferramenta — mesmo risco de colisão do EasyPanel citado abaixo, já que ele também tende a usar 3000, mas fora do nosso controle igual), Loki `3100` (padrão da ferramenta), Zabbix web `8084`, SigNoz `8085` (frontend, remapeado do `8080` padrão via patch — mesma colisão do Traefik) + `4317`/`4318` (ingestão OTLP grpc/http, mesmos números padrão da ferramenta, só o bind que vira loopback). Coolify/EasyPanel/cPanel escolhem a própria porta via instalador deles, fora do nosso controle. Restic não tem porta — é só backup agendado (systemd timer), sem UI web. `19999` ficou livre de novo (era do Netdata, removido).

**Redes docker dos extras**: cAdvisor/Prometheus/Grafana/Loki entram todos na mesma rede `server-safe-monitoring` (criada com `docker network create ... || true` idempotente em cada um dos 4 scripts, não centralizado — é só uma linha, não vale promover pra common.sh) — permite resolver os outros pelo nome do container (`http://prometheus:9090`, `http://loki:3100`) na hora de configurar datasource no Grafana manualmente; nenhum desses 4 scripts configura o datasource/scrape target dos outros automaticamente, cada extra continua independente. Zabbix usa a própria rede isolada `server-safe-zabbix` (Postgres + server + web, 3 containers) — sem relação com a rede de monitoring, é uma stack própria. SigNoz também é isolado (`signoz-network`, criada pelo próprio compose que o `foundryctl` gera) — não teria como entrar em `server-safe-monitoring` mesmo se quisesse, é uma stack própria (ClickHouse + Postgres + collector) sem relação com os outros 4.

SigNoz (`signoz-install.sh`) é o extra mais pesado do grupo (6 containers: ClickHouse, Postgres, collector/`ingester`, migrator, keeper, frontend — recomenda 4GB de RAM livres) e o único instalado via ferramenta oficial de terceiros em vez de `docker run`/vendor-script direto: usa `foundryctl` (CLI do projeto SigNoz, sucessora do antigo `docker-compose` direto que a própria SigNoz descontinuou — `deploy/install.sh` deles hoje só imprime aviso de depreciação e sai). Fluxo: instala `foundryctl` em `/usr/local/bin` se faltar (`curl https://signoz.io/foundry.sh | bash`), escreve `/etc/server-safe/signoz/casting.yaml` (`metadata.name: signoz` fixo — é isso que determina o nome dos containers gerados, ex. `signoz-signoz-0`, então **não mudar esse nome sem atualizar os checks de idempotência do script junto**) com três `spec.patches` (JSON Patch RFC 6902) contra o `deployment/compose.yaml` gerado, pra forçar bind em loopback nas 3 portas publicadas (frontend `8080→127.0.0.1:8085`, ingester `4317`/`4318→127.0.0.1`) — sem os patches, o compose gerado publica em `0.0.0.0` por padrão, o que quebraria a política de "todo admin/ingest é loopback-only" deste projeto. Depois roda `foundryctl cast -f casting.yaml -p pours` (gauge+forge+deploy num passo só). Idempotência: checa `docker ps -a` pelo nome fixo do container do frontend (`signoz-signoz-0`) — se já existe, só garante que os containers estão rodando (`docker start`), não regera o casting nem roda `cast` de novo.

Zabbix é o extra mais pesado (3 containers, não 1) — segue a mesma senha de banco gerada uma vez e persistida (`/etc/zabbix/db-password`, `600`, mesmo idioma do Restic) pra reaproveitar entre containers e reruns. Sem o healthcheck com `depends_on: condition: service_healthy` que o docker-compose oficial do Zabbix usa (aqui é `docker run` sequencial, não compose), o `zabbix-web` pode reclamar de conexão por 1-2min no primeiro boot até o `zabbix-server` terminar de criar o schema — `--restart=always` cobre isso, não é bug, só timing. Login inicial documentado na UI: `Admin/zabbix` (Zabbix) e `admin/admin` (Grafana) — trocar no primeiro acesso.
...
`cleanup-schedule.sh` (etapa 9) tem duas opções Docker além do self-destruct: `SS_STOP_CONTAINERS` (`docker stop` em tudo que estiver rodando) e `SS_REMOVE_IMAGES` (para+remove **todos** os containers com `docker rm -f`, depois todas as imagens com `docker rmi -f` — remover imagem exige que nenhum container, nem parado, esteja usando ela, então essa opção sempre remove containers também, não só parar). Diferente do self-destruct/paths (que ficam agendados com atraso via `systemd-run`), esses dois rodam **na hora**, síncrono, log ao vivo — e `|| true` depois de cada `xargs` pra não abortar o script inteiro (`set -e`) se um container/imagem específico falhar ao remover. Não mexe em volumes/networks nomeados (dados de containers sobrevivem). Como agora dá pra pedir só a limpeza Docker sem self-destruct nem paths, o guard antigo (`fail()` se não tinha nada pra agendar) virou um `if`: só falha se **nada** foi pedido — nem self-destruct, nem paths, nem limpeza Docker. UI: os dois checkboxes vêm desmarcados por padrão e com `confirm()` no `app.js` antes de mandar a requisição — afetam a máquina inteira, não só o que o wizard instalou.
