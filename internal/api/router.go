...
func RegisterRoutes(mux *http.ServeMux, ks *keystore.Store, pt *progress.Tracker, version string) {
	...
	mux.HandleFunc("POST /api/extras/uptimekuma", h.InstallUptimeKuma)
	mux.HandleFunc("POST /api/extras/cadvisor", h.InstallCadvisor)
	mux.HandleFunc("POST /api/extras/prometheus", h.InstallPrometheus)
	...
	mux.HandleFunc("POST /api/extras/nodeexporter", h.InstallNodeExporter)
